...
def query_knowledge_graph(equipment_tag: str) -> str:
    ...
    try:
        # 1. Get Node Info
        node_res = (
            supabase.table("knowledge_nodes").select("*").eq("tag", tag).execute()
        )
        ...
        edges_res = (
            ...
            .select("*, target:target_id(tag, name)")
        ...
        expert_res = (
            supabase.table("expert_captures")
            .select("*")
        ...
        for exp in expert_res.data:
            result.append(
                f"Expert Note ({exp['expert_name']}): {exp['knowledge_text']}"
            )
        ...
        if tag:
            ...
            try:
                supabase.table("knowledge_nodes").upsert(
                    {"tag": tag, "name": tag, "type": "Equipment", "properties": {}},
                    on_conflict="tag",
                ).execute()
            except Exception:
                pass
        ...
        for edge in extracted.get("edges", []):
            try:
                source_res = (
                    ...
                    .select("id")
                    .eq("tag", edge["source_tag"].upper())
                    .execute()
                )
                target_res = (
                    supabase.table("knowledge_nodes")
                    .select("id")
                    .eq("tag", edge["target_tag"].upper())
                    .execute()
                )
...
def ask_copilot(query: str) -> dict:
    if not groq_client:
        return {
            "reasoning_steps": [],
            "response": "Groq API key is not configured in the environment.",
        }
    ...
    messages = [
        {
            "role": "system",
            "content": "You are the IntelliPlant Copilot, a highly knowledgeable industrial engineer AI.",
        },
            ...
            for tool_call in tool_calls:
                ...
                if function_name == "query_knowledge_graph":
                    ...
                    reasoning_steps.append(
                        f"Querying Knowledge Graph for equipment '{tag}'..."
                    )
